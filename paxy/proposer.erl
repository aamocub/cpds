...
collect(_, 0, _, _, _) ->
  abort;
collect(Promises, Sorrys, Round, MaxVoted, Proposal) ->
  receive
    {promise, Round, _, na} ->
      collect(Promises-1, Sorrys, Round, MaxVoted, Proposal);
    {promise, Round, Voted, Value} ->
      case order:gr(Voted, MaxVoted) of
        true ->
          collect(Promises-1, Sorrys, Round, Voted, Value);
        false ->
          collect(Promises-1, Sorrys, Round, MaxVoted, Proposal)
      end;
    {promise, _, _,  _} ->
      collect(Promises, Sorrys, Round, MaxVoted, Proposal);
    {sorry, {prepare, Round}} ->
      collect(Promises, Sorrys-1, Round, MaxVoted, Proposal);
    {sorry, _} ->
      collect(Promises, Sorrys, Round, MaxVoted, Proposal)
  after ?timeout ->
    abort
...
vote(_, 0, _) ->
  abort;
vote(Votes, Sorrys, Round) ->
  receive
    {vote, Round} ->
      vote(Votes-1, Sorrys, Round);
    {vote, _} ->
      vote(Votes, Sorrys, Round);
    {sorry, {accept, Round}} ->
      vote(Votes, Sorrys-1, Round);
    {sorry, _} ->
      vote(Votes, Sorrys, Round)
  after ?timeout ->
    abort
