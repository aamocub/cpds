...
\begin{itemize}
\item They were defined two constants with the server host name and the start timeout respectively.
\item It was implemented a \textit{startServer} function to be executed in the remote environment. This function just called the start function  of the server, registered it in the local instance, and sent a \textit{started} message back to the caller.
\item The \textit{spawn} function was used to execute the previously described \textit{startServer} function.
\item It was implemented a \textit{waitServer} function to await for the server \textit{started} message or abort after a timeout otherwise.
\end{itemize}

With the previous modification, the distributed execution was achieved. It can be evidenced in the following image:

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{img/remote_success.png}
    \caption{Distributed execution running succesfully}
    \label{fig:distributed_ok}
\end{figure}

In the same way, if we kill the \textit{opty-srv} instance, it can be verified the distributed execution doesn't work anymore:
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\linewidth]{img/remote_fail.png}
    \caption{Distributed execution failing}
    \label{fig:distributed_fail}
\end{figure}

Regardig the question about where is the handler running, it runs in the same instance as the clients. The reason for this is that the only function exported in the \textit{handler} module is the \textit{start} function, and this function is only called inside \textit{client:open}, which is part of the internal functionality of the \textit{client} module, therefore, the handler will run wherever the client module is being executed.
