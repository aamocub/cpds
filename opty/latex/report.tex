...
Unless otherwise specified, the default parameters are 5 clients, 50 entries, 1 read and 1 write per transaction, and 5 seconds of execuction time.

The percentage of successful transactions shown is calculated as the average of the success rate of every client.
It is important to note that the success rate is almost the same for all clients in every single experiment (standard deviation is \(< 0.5\%\) in all cases).
